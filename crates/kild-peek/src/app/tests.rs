...
fn test_cli_screenshot_window() {
    let app = build_cli();
    let matches = app.try_get_matches_from(vec!["kild-peek", "screenshot", "--window", "Terminal"]);
    assert!(matches.is_ok());
...
fn test_cli_key_with_app() {
    let app = build_cli();
    let matches = app.try_get_matches_from(vec!["kild-peek", "key", "--app", "TextEdit", "enter"]);
    assert!(matches.is_ok());
...
fn test_cli_elements_with_window() {
    let app = build_cli();
    let matches = app.try_get_matches_from(vec!["kild-peek", "elements", "--window", "Terminal"]);
    assert!(matches.is_ok());
